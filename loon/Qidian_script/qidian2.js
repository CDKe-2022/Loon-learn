/* 🥳 脚本功能: 自动观看 起点读书 阅读页广告 (任务3: 广告·加点！)
   ...
   核心特性: 防并发锁 / 深拷贝防污染 / 致命错误提前终止 / 失败保留数据 / 动态适配多组
   默认间隔时间: 0.5s (可通过 qd_timeout 修改) */

const CONFIG = {
  // 核心修改：改为动态读取，适配6个或更多子任务
  ...
  LOCK_KEY: "qd_reading_lock" // 防并发脚本锁
};
...
function cloneRequestOptions(options) {
  // 使用深拷贝彻底杜绝底层库对嵌套对象（如 policy/retry）的污染
  try {
...
(async () => {
  ...
  if (totalGroups === 0) {
    console.log("🟡未找到任何阅读任务ID，请先去福利中心抓包获取");
    $notification.post(CONFIG.NOTIFICATION_TITLE, "⚠️无数据", "未找到广告·加点任务ID");
    return;
  }

  for (let i = 0; i < totalGroups; i++) {
    const sessionKey = `${CONFIG.READING_SESSION_PREFIX}${i + 1}`;
    const sess = $persistentStore.read(sessionKey);
    if (sess) {
      currentSessionStr = sess;
      currentSessionKey = sessionKey;
      currentGroupLabel = `任务3-组${i + 1}/${totalGroups}(加点)`;
      break;
    }
  }
