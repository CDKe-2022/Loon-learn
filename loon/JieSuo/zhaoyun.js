/*************************************
应用名称：赵云与阿斗-小游戏
脚本功能：辅助小工具
更新日期：2026-07-11
脚本作者：@ddm1023
电报频道：https://t.me/ddm1023
使用声明：⚠️仅供参考，🈲转载与售卖！
*************************************/

// Loon参数获取
function getArgument(key, def) {
  try {
    if (typeof $argument !== 'undefined' && $argument[key] !== undefined) {
      const value = $argument[key];
      // 布尔值处理
      if (typeof value === 'boolean') return value;
      // 数字处理
      if (typeof value === 'number' && !isNaN(value)) return value;
      // 字符串处理
      if (typeof value === 'string') {
        // 检查是否为布尔字符串
        if (value === 'true') return true;
        if (value === 'false') return false;
        // 尝试解析为数字
        const num = Number(value);
        if (!isNaN(num) && value.trim() !== '') return num;
        // 尝试解析为JSON数组
        try {
          const parsed = JSON.parse(value);
          if (Array.isArray(parsed)) return parsed;
        } catch (e) { }
        // 普通字符串
        return value;
      }
      return value;
    }
  } catch (e) {
    console.log(`读取参数失败: ${key}`, e.message);
  }
  return def;
}

// 获取布尔值参数
function getBoolArg(key, def = false) {
  const value = getArgument(key, def);
  return value === true || value === 'true';
}

// 获取数字参数
function getNumberArg(key, def) {
  const value = getArgument(key, def);
  const num = Number(value);
  return isNaN(num) ? def : num;
}

// 获取多选参数（技能）
function getCheckboxArg(key, def = []) {
  const value = getArgument(key, def);
  // 已经是数组
  if (Array.isArray(value)) {
    return value;
  }
  // 字符串尝试解析
  if (typeof value === 'string') {
    try {
      const parsed = JSON.parse(value);
      if (Array.isArray(parsed)) return parsed;
    } catch (e) { }
    // 逗号分隔的字符串
    if (value.includes(',')) {
      return value.split(',').map(s => s.trim()).filter(Boolean);
    }
    // 单个值
    if (value.trim()) return [value.trim()];
  }
  return def;
}

// 参数读取
const ForceValue = getBoolArg("ddm_zyyad_forcevalue", false);
const EnableGold = getBoolArg("ddm_zyyad_enablegold", true);
const EnableLevel = getBoolArg("ddm_zyyad_enablelevel", true);
const EnableWeapon = getBoolArg("ddm_zyyad_enableweapon", true);
const EnableSkill = getBoolArg("ddm_zyyad_enableskill", true);
const EnableAvatar = getBoolArg("ddm_zyyad_enableavatar", true);
const EnableRegister = getBoolArg("ddm_zyyad_enableregister", true);

const Gold = getNumberArg("ddm_zyyad_gold", 9999999);
const RegisterDay = getNumberArg("ddm_zyyad_registerday", 7);
const Level = getNumberArg("ddm_zyyad_level", 999);
const WeaponMode = getNumberArg("ddm_zyyad_weaponmode", 2);
const WeaponCount = getNumberArg("ddm_zyyad_weaponcount", 50);

// 技能参数读取（多选框用多个参数模拟）
const ActiveSkill1 = getCheckboxArg("ddm_zyyad_active1", []);
const ActiveSkill2 = getCheckboxArg("ddm_zyyad_active2", []);
const ActiveSkill = [...ActiveSkill1, ...ActiveSkill2].slice(0, 2);

const PassiveSkill1 = getCheckboxArg("ddm_zyyad_passive1", []);
const PassiveSkill2 = getCheckboxArg("ddm_zyyad_passive2", []);
const PassiveSkill3 = getCheckboxArg("ddm_zyyad_passive3", []);
const PassiveSkill4 = getCheckboxArg("ddm_zyyad_passive4", []);
const PassiveSkill5 = getCheckboxArg("ddm_zyyad_passive5", []);
const PassiveSkill6 = getCheckboxArg("ddm_zyyad_passive6", []);
const PassiveSkill = [...PassiveSkill1, ...PassiveSkill2, ...PassiveSkill3, 
                        ...PassiveSkill4, ...PassiveSkill5, ...PassiveSkill6].slice(0, 6);

// 环境检测（Loon环境）
const isLoon = typeof $loon !== 'undefined';

// 安全JSON解析
function safeJson(body) {
  try {
    return JSON.parse(body || "{}");
  } catch (e) {
    return {};
  }
}

// 设置数值（安全模式）
function setValue(data, key, value) {
  if (ForceValue) {
    data[key] = value;
    return;
  }
  const old = Number(data[key]) || 0;
  if (old < value) {
    data[key] = value;
  }
}

// 当前时间戳
const current = Date.now();
const headers = $request ? ($request.headers || {}) : {};

// 武器碎片列表
const WeaponList = [[1,50],[2,50],[3,50],[4,50],[5,50],[6,50],[7,50],[8,50],[9,50],[11,50],[12,50],[13,50],[14,50],[15,50],[16,50],[17,50],[18,50],[19,50],[20,50],[22,50],[23,50],[24,50],[25,50],[26,50],[27,50],[28,50],[29,50],[30,50],[32,50],[33,50],[34,50],[35,50],[36,50],[37,50],[38,50],[39,50],[40,50],[41,50],[42,50],[43,50]];

// 技能配置
const SkillName = {
  2:"毛笔",
  3:"练兵符",
  4:"神兵符",
  5:"包子",
  6:"御敌千里",
  7:"砚台",
  8:"陷阱",
  9:"地雷",
  10:"速攻符",
  11:"降妖符",
  12:"农民",
  13:"招贤榜",
  14:"攻速符(全体)",
  15:"齐头并进",
  16:"续命丹",
  17:"大补丸",
  18:"泥潭",
  19:"洛阳铲",
  20:"召唤陨石",
  21:"垃圾桶",
  22:"升职令",
  24:"摸金校尉"
};

// 禁止技能
const BanSkill = [1, 23];

// 技能列表生成
const SkillList = [];
const SkillUsed = Object.create(null);

function AddSkill(id) {
  id = Number(id);
  if (!id) return;
  if (SkillList.length >= 8) return;
  if (BanSkill.includes(id)) {
    console.log(`过滤特殊技能: ${SkillName[id] || id}`);
    return;
  }
  if (SkillUsed[id]) {
    console.log(`忽略重复技能: ${SkillName[id] || id}`);
    return;
  }
  SkillUsed[id] = true;
  SkillList.push([id]);
}

// 添加主动技能
ActiveSkill.slice(0, 2).forEach(function(id) {
  AddSkill(id);
});

// 添加被动技能
PassiveSkill.slice(0, 6).forEach(function(id) {
  AddSkill(id);
});

// 默认用户数据模板
const UserTemplate = {
  pv: "",
  nk: "",
  lld: 0,
  rt: current,
  ss: 0,
  lst: 0,
  wn: 0,
  st: current,
  lp: [],
  sg: {
    iosBetterMode: true,
    soundVolume: 0,
    showDamageNum: false,
    musicVolume: 0
  },
  afu: false,
  apsc: 0,
  wdg: true,
  ld: 0,
  cld: 1,
  ls: 0,
  wd: 0,
  sac: 0,
  au: "resources/img/mainUI/avatar/avatar15.png",
  op: true,
  ws: 0,
  cs: 1,
  lrt: 0,
  wf: [],
  eq : [19,13,18,29,6,26,29,18,43,43,43,8],
  lts: 0,
  ga: 0,
  mg: [],
  sm: 30,
  hfb: true,
  ot: current,
  aul: Array(16).fill(1),
  ps: [],
  ssc: 0,
  gd: 0,
  wfr: true,
  hfs: true,
  nwi: [],
  fds: 0,
  rr: 1,
  pap: true,
  rc: 1
};

// 初始化用户数据
function InitUserData(data) {
  if (!data || typeof data !== "object") {
    data = {};
  }
  Object.keys(UserTemplate).forEach(function(key) {
    if (typeof data[key] === "undefined" || data[key] === null) {
      let value = UserTemplate[key];
      if (Array.isArray(value)) {
        data[key] = JSON.parse(JSON.stringify(value));
      } else if (typeof value === "object") {
        data[key] = JSON.parse(JSON.stringify(value));
      } else {
        data[key] = value;
      }
    }
  });
  return data;
}

// 功能模块
function Module(data, attach) {
  if (!data) return;
  Object.assign(data, {
    "hfb": true, // 新手引导
    "pap": true, // 隐私协议
    "wfr": true, // 武器功能
    "sm": 30 // 体力值
  });
  
  // 金币模块
  if (EnableGold) {
    setValue(data, "gd", Gold);
  }
  
  // 注册时间模块
  if (EnableRegister) {
    setValue(data, "lld", RegisterDay);
    ModuleRegister(data, attach);
  }
  
  // 等级模块
  if (EnableLevel) {
    ModuleLevel(data);
  }
  
  // 武器模块
  if (EnableWeapon) {
    ModuleWeapon(data);
  }
  
  // 技能模块
  if (EnableSkill) {
    ModuleSkill(data);
  }
  
  // 头像模块
  if (EnableAvatar) {
    ModuleAvatar(data);
  }
}

// 注册时间模块
function ModuleRegister(data, attach) {
  const target = current - RegisterDay * 86400000 - 300000;
  if (ForceValue || !data.rt || data.rt > target) {
    data.rt = target;
  }
  if (attach && typeof attach === "object") {
    if (ForceValue || !attach.ct || attach.ct > target) {
      attach.ct = target;
    }
  }
}

// 等级模块
function ModuleLevel(data) {
  setValue(data, "cs", Level); // 等级
  setValue(data, "ga", Level); // 昨日等级
  setValue(data, "wn", Level); // 胜利次数
  setValue(data, "ws", Level); // 连胜次数
  setValue(data, "ls", Level); // 历史最高
  setValue(data, "cld", Math.max(99, Math.ceil(Level / 10))); // 章节
}

// 武器模块
function ModuleWeapon(data) {
  if (WeaponMode === 0) return;
  
  if (!Array.isArray(data.wf)) {
    data.wf = [];
  }
  
  // 模式1：自动补充已有武器碎片
  if (WeaponMode === 1) {
    data.wf.forEach(function (item) {
      if (Array.isArray(item) && item.length > 1) {
        item[1] = Math.max(Number(item[1]) || 0, WeaponCount);
      }
    });
    return;
  }
  
  // 模式2：导入武器（保留已有，重复覆盖）
  const WeaponMap = {};
  
  // 建立已有武器索引
  data.wf.forEach(function (item) {
    if (Array.isArray(item) && item.length > 1) {
      WeaponMap[item[0]] = item;
    }
  });
  
  // 导入武器
  WeaponList.forEach(function (item) {
    if (WeaponMap[item[0]]) {
      // 已存在
      WeaponMap[item[0]][1] = ForceValue ? item[1] : Math.max(Number(WeaponMap[item[0]][1]) || 0, item[1]);
    } else {
      // 不存在
      data.wf.push([item[0], item[1]]);
    }
  });
}

// 技能模块
function ModuleSkill(data) {
  if (!Array.isArray(data.ps)) {
    data.ps = [];
  }
  data.ps = SkillList.map(function(v, index) {
    return [
      v[0],
      1,
      current - (index + 1) * 300000
    ];
  });
}

// 头像模块
function ModuleAvatar(data) {
  if (!Array.isArray(data.aul)) {
    data.aul = Array(16).fill(1);
  } else {
    data.aul = data.aul.map(() => 1);
  }
}

// 主逻辑
let obj = {};
let ddm = {};
let data = {};
let attach = {};

if (typeof $response === "undefined") {
  // 请求拦截（用于分享接口）
  ddm = safeJson($request.body);
  data = ddm;
  Module(data, null);
  obj.body = JSON.stringify(ddm);
  $done(obj);
} else {
  // 响应拦截（用于登录和用户数据接口）
  ddm = safeJson($response.body);

  if (/user\/login/.test($request.url)) {
    ddm.data = ddm.data || {};
    if (!ddm.data.userData || typeof ddm.data.userData !== "object") {
      ddm.data.userData = {};
    }
    ddm.data.userData = InitUserData(ddm.data.userData);
    
    if (!ddm.data.attach || typeof ddm.data.attach !== "object") {
      ddm.data.attach = {};
    }
    if (!ddm.data.info || typeof ddm.data.info !== "object") {
      ddm.data.info = {};
    }
    
    data = ddm.data.userData;
    attach = ddm.data.attach;
    Module(data, attach);
    
    ddm.data.userType = 1;
    ddm.code = 0;
    ddm.msg = "Success";
  }

  // 用户数据接口
  if (/user\/data/.test($request.url)) {
    if (ddm.data && ddm.data.userData && typeof ddm.data.userData === "object") {
      data = ddm.data.userData;
      Module(data, null);
    }
  }

  // 分享接口
  if (/toutiaoGame\/ZhaoYunAndADou/.test($request.url)) {
    ddm.shareLimitPerDay = Math.max(ddm.shareLimitPerDay || 0, 99);
  }

  obj.status = 200;
  obj.body = JSON.stringify(ddm);
  $done(obj);
}
